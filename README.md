...
- `베타시즌 실행.cmd`: 베타 시즌 2 전용 페이지로 접속
...
- 베타 시즌 2 운영 기간은 2026년 8월 3일부터 8월 10일까지 1주일입니다.
- 신규 캐릭터 Gold는 900 β 크레딧으로 해금합니다.
- Gold의 일반 공격은 2×2타일 정십이면체 금광석을 직선으로 8타일 발사하며, 적·벽·최대 사거리에 닿으면 2단계로 분열하고 각 2단계 투사체가 다시 6개의 금괴 모형으로 분열합니다.
- Gold의 궁극기 `고장 지대`는 반경 3타일 장판을 4초 동안 생성해 일반 공격과 특수 공격을 막고 이동 속도를 50% 감소시킵니다.
- Pink의 신규 궁극기 이름은 `앙코르!`이며, 범위 안에서 쓰러진 아군을 사망 위치에 체력 40%와 2초 무적으로 부활시킵니다.
- `골드 러쉬 시작` 버튼으로 중앙 금광 전용 테스트 모드를 실행할 수 있습니다.
- 골드 러쉬는 자연 생성 금 최대 20개, 금 10개를 10초간 보유하면 승리, 사망 시 금 전부 드롭, 5초 후 최초 스폰 지점 부활, 부활 직후 2초 무적 규칙을 사용합니다.
...
- 상세 기획은 `specs/beta-season-2.md`에서 확인할 수 있습니다.
