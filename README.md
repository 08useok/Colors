...
- 현재 메인 버전: `v1.5.4.1`
- 현재 시즌: 베타 시즌 4
- 다음 시즌 테스트: 베타 시즌 5 (`?test=beta5`)
- 빌드 도구 없이 ES 모듈과 Three.js CDN을 사용합니다.

## 실행 파일로 바로 접속하기

프로젝트 폴더에 있는 실행 파일을 더블클릭하면 로컬 서버가 자동으로 켜지고 브라우저가 열립니다.

- `게임 실행.cmd`: 기본 게임으로 접속
- `베타시즌 실행.cmd`: 베타 시즌 5 전용 테스트 페이지로 접속

서버가 이미 실행 중이면 새 서버를 만들지 않고 해당 페이지를 바로 엽니다.

### 직접 CMD에서 실행하기

실행 파일을 사용하지 않을 때는 `Win + R`을 누르고 `cmd`를 입력한 뒤 다음 명령을 실행합니다.
...
CMD 창을 닫지 않은 상태에서 브라우저 주소창에 원하는 주소를 입력합니다.
...
- 신규 영웅 캐릭터 `Mint`를 테스트할 수 있습니다.
- Mint의 일반 공격 `아이스크림 탄`은 3발을 연속 발사하고, 적중할 때마다 얼음 수치를 누적해 대상을 빙결시킵니다.
- Mint의 특수 공격 `아이스크림 장판`은 10타일 지점에 반경 9타일 장판을 10초간 생성합니다. 장판 위 적은 매초 300 피해와 얼음 수치 5를 받고 점점 빠르게 미끄러집니다.
- Blue의 특수 공격 `돌진`은 전방으로 빠르게 굴러가며 벽에 횟수 제한 없이 튕기고, 경로의 적에게 피해와 넉백을 줍니다. 일반 공격 적중 4회로 충전합니다.
- 시즌 5의 쇼다운은 `SHOWDOWN+`로 대체됩니다. 생존 등수는 그대로 유지하면서 플레이어가 적을 처치할 때마다 트로피 +2를 추가로 받습니다.
- 캐릭터별 트로피·승수·누적 처치 수와 한 판 최고 처치 수를 시즌 5 전용 저장소에 기록합니다.
- 놀이공원 전용 맵 색상, 장식, 로딩 이미지와 음악 `Clockwork Midway`가 적용됩니다.
- Pink 전용 초희귀 스킨 `솜사탕 핑크`가 추가됐으며, 전용 FBX 모델과 걷기 시작·반복·정지 모션을 사용합니다.
...
- Mint 특수 공격을 조준하는 동안 일반 공격 조준선은 숨겨지고, 투척 방향선과 장판 착지 범위만 표시됩니다.
- GLB 캐릭터는 캐릭터별 공격 포즈를 사용하며, Mint는 3발 각각의 발사 순간에 공격 동작을 재생합니다.

### 테스트 도구

- 상단 `에셋 쇼룸`에서 GLB 모델을 직접 열어 회전·확대하고 포함된 애니메이션을 재생할 수 있습니다.
- 캐릭터 선택, 시즌 상점, 일일 보상, 골드 러쉬와 SHOWDOWN+를 한 페이지에서 시험할 수 있습니다.
- 베타 시즌 5 상태는 `colorsBetaSeason5Test` 키에 저장되며 이전 시즌과 분리됩니다.
- 상세 구현 기준은 `specs/beta-season-5.md`, 밸런스 기준은 `specs/beta-season-4-balance-patch.md`에서 확인할 수 있습니다.

## 코드 설명서

### 쉬운 이름 태그

코드에서 영어 이름을 발견하면 아래 한국어 태그로 이해하면 됩니다.

| 코드 이름 | 쉬운 태그 | 역할 |
|---|---|---|
| `state` | 게임 상태 저장소 | 현재 플레이어, 투사체, 맵과 전투 상태를 모아 둡니다. |
| `scene` | 3D 무대 | 화면에 보일 모든 3D 객체가 들어갑니다. |
| `camera` | 게임 카메라 | 플레이어가 보는 위치와 방향을 정합니다. |
| `renderer` | 화면 출력기 | 3D 무대를 브라우저 캔버스에 그립니다. |
| `createMap` | 맵 생성기 | 선택된 맵 데이터로 바닥, 벽, 수풀과 장식물을 만듭니다. |
| `clearBattleMap` | 맵 청소기 | 이전 맵의 객체와 충돌 데이터를 제거합니다. |
| `MAP_POOL` | 맵 설계도 모음 | 사용할 수 있는 모든 맵 배치를 저장합니다. |
| `wallSpecs` | 벽 배치표 | 벽의 위치와 크기를 숫자 배열로 저장합니다. |
| `createWall` | 벽 생성기 | 보이는 벽과 벽 충돌 영역을 함께 만듭니다. |
| `battleSolids` | 벽 충돌 목록 | 현재 전투 맵에서 이동을 막는 구조물을 저장합니다. |
| `solids` | 현재 충돌 목록 | 현재 게임 모드가 사용하는 충돌 구조물을 가리킵니다. |
| `bushSpecs` | 수풀 배치표 | 수풀을 놓을 좌표를 저장합니다. |
| `createBush` | 수풀 생성기 | 여러 잎 덩어리와 은신 범위를 가진 수풀을 만듭니다. |
| `battleBushes` | 전투 수풀 목록 | 현재 전투 맵의 수풀 위치, 크기와 3D 객체를 저장합니다. |
| `bushes` | 현재 수풀 목록 | 현재 게임 모드에서 은신 판정에 사용할 수풀을 가리킵니다. |
| `isInBush` | 수풀 진입 검사기 | 캐릭터가 수풀 범위 안에 있는지 확인합니다. |
| `isVisibleThroughBush` | 수풀 시야 검사기 | 두 캐릭터 사이의 수풀이 시야를 막는지 확인합니다. |
| `isFighterVisible` | 캐릭터 노출 검사기 | 거리, 수풀과 발각 시간을 종합해 대상이 보이는지 결정합니다. |
| `account` | 계정 정보 묶음 | 닉네임, 재화, 전적, 캐릭터와 스킨 정보를 저장합니다. |
| `coins` | 일반 재화 | 계정이 보유한 코인 수입니다. |
| `loadAccount` | 계정 불러오기 | 브라우저 저장소에서 현재 계정을 읽습니다. |
| `saveAccount` | 계정 저장기 | 변경된 계정과 재화를 브라우저 저장소에 기록합니다. |
| `loadAccountRegistry` | 계정 목록 불러오기 | 브라우저에 저장된 전체 계정 목록을 읽습니다. |
| `ACCOUNTS_KEY` | 계정 목록 저장 키 | `localStorage`의 전체 계정 목록 이름입니다. |
| `ACTIVE_ACCOUNT_KEY` | 활성 계정 저장 키 | 현재 탭에서 사용하는 계정 ID의 이름입니다. |
| `projectiles` | 투사체 목록 | 현재 날아가는 모든 공격 투사체를 저장합니다. |
| `effects` | 효과 목록 | 불꽃, 참격과 피해 표시처럼 잠시 나타나는 효과를 저장합니다. |
| `updateEffects` | 효과 갱신기 | 효과의 움직임과 남은 수명을 매 프레임 처리합니다. |

예를 들어 `createWall(-18, 40, 8, 2)`는 **벽 생성기**를 사용해 `(-18, 40)` 위치에 `8 × 2` 크기의 벽을 만든다는 뜻입니다.
