...
### 동작 조건
- 마지막 전투 행동(공격 또는 피격)으로부터 **플레이어 3초 / 봇 5초** 경과 시, **최대 체력의 25%를 즉시(한 번에) 회복**
...
- 전투 행동(피격/공격) 시 `lastCombatTime` 갱신으로 대기 조건이 다시 false가 되어 회복 사이클이 중단됨

### 적용 대상
- 플레이어 + 봇 전원 (단, 봇의 대기시간은 5초로 플레이어보다 길다)
...
- [ ] 봇은 5초간 피격/공격 없으면 최대 체력의 25%가 즉시 한 번에 회복된다
